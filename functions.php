...
function get_four_products() {
	$query = query("SELECT * FROM products LIMIT 3");
	$count = 0;
	confirm($query);
	...
	while ($row = fetch_array($query)) {
		...
		if ($count < 8) {
			$product = <<<DELIMETER
		    <div class="col-4">
		    	<a href="product_detail.php?id={$row['product_id']}"><img src="images/{$row['product_image']}"></a>
		    	<h4><a href="product_detail.php?id={$row['product_id']}">{$row['product_title']}</a></h4>
		    	<p style="color: red;">RM{$row['product_price']}</p>
...
function get_products() {
	...
	while ($row = fetch_array($query)) {
		...
		$product = <<<DELIMETER
		    <div class="col-4">
		    	<a href="product_detail.php?id={$row['product_id']}"><img width='100' src="images/{$row['product_image']}"></a>
		    	<h4><a href="product_detail.php?id={$row['product_id']}">{$row['product_title']}</a></h4>
		    	<p style="color: red;">RM{$row['product_price']}</p>
		    	<p>{$row['product_short_desc']}</p>
		        <a class="btn" href="cart.php?add={$row['product_id']}">Add to cart</a>
...
function get_products_update_page() {
	...
	while ($row = fetch_array($query)) {
		...
		<tr>
		    ...
		    <td><a href="product_detail.php?id={$row['product_id']}"><img src="images/{$row['product_image']}" width=100></td></a>
...
function get_products_admin_page() {
	...
	while ($row = fetch_array($query)) {
		...
		<tr>
		    <td>{$row['product_id']}</td>
		    <td><img src="images/{$row['product_image']}" width=100></td>
